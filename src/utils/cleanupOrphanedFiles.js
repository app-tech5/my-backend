...
async function deleteOrphanedFiles() {
  try {
    ...
    // Étape 2: Récupérer tous les modèles avec un champ `image` ou `documents`
    ...
    // Étape 3: Récupérer toutes les URLs référencées dans la base de données
    console.log('Récupération des fichiers référencés dans la base de données...');
    const usedFiles = new Set(); // Utilisation d'un Set pour éviter les doublons
    ...
    // Étape 4: Supprimer les fichiers orphelins
    console.log('Début de la suppression des fichiers orphelins...');
    files.forEach(file => {
